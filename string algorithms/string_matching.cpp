...
class solution
{
    ...
    void solve()
    {
       ...
       vector<long long> hash1(s.size()),hash2(t.size());
       hash1[0]=(s[0]-'a');
       hash2[0]=(t[0]-'a');
        ...
        {
            hash2[i]=((t[i]-'a')*p_pow1[i])%m1;   
            hash2[i] = (hash2[i] + (i > 0 ? hash2[i-1] : 0LL)) % m1;
        }
        long long ans=0;
        //bug(hash2[t.size()-1]);
        for(int i=0;i<=s.size()-t.size();i++)
        {
            long long h=substrhash(i,i+t.size()-1,hash1);
            //bug(h);
            if(h==hash2[t.size()-1]) ans+=1;
        }
        cout<<ans<<endl;
